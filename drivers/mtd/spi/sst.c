...
static int
sst_unlock_global_block_protection(struct spi_flash *flash)
{
	int ret;
	...
	ret = spi_flash_cmd(flash->spi, CMD_SST_ULBPR, NULL, 0);
	if (ret) {
		debug("SF: Unlocking Global Block protection failed\n");
	}
...
static int
sst_write_02(struct spi_flash *flash, u32 offset, size_t len, const void *buf)
{
	size_t actual, cmd_len;
	int ret;
	u8 cmd[4];
	...
	ret = -1;
	printf("sst_write is not implemented\n");
...
spi_flash_probe_sst(struct spi_slave *spi, u8 *idcode)
{
	...
	if (0x42 == idcode[2]) {
		sst_unlock_global_block_protection(&stm->flash);
	} else {
