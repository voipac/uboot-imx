...
#define UNLOCK_G_PROTECTION(a)	 spi_nor_cmd_1byte(a, ULBPR)
...
static int spi_nor_flash_write_02(struct spi_flash *flash, u32 offset,
		size_t len, const void *buf)
{
	struct imx_spi_flash *imx_sf = to_imx_spi_flash(flash);
	...
	for (actual = 0; actual < len; actual += chunk_len) {
		...
		chunk_len = min(chunk_len, 128);

		{
			size_t data_len = chunk_len;
			u8 * pTxBuffer = &g_tx_buf[4];
			const u8 * pInBuffer = ((const u8 *) buf) + actual;

			switch (data_len % 4) {
			case 0:
				g_tx_buf[3] = BYTE_PROG;
				g_tx_buf[2] = (write_addr >> 16) & 0xff;
				g_tx_buf[1] = (write_addr >> 8) & 0xff;
				g_tx_buf[0] = (write_addr >> 0) & 0xff;

				data_len -= 0;
				pTxBuffer = &g_tx_buf[4];
				break;
			case 1:
				g_tx_buf[0] = BYTE_PROG;

				g_tx_buf[7] = (write_addr >> 16) & 0xff;
				g_tx_buf[6] = (write_addr >> 8) & 0xff;
				g_tx_buf[5] = (write_addr >> 0) & 0xff;
				g_tx_buf[4] = *(pInBuffer++);

				data_len -= 1;
				pTxBuffer = &g_tx_buf[8];
				break;
			case 2:
				g_tx_buf[1] = BYTE_PROG;
				g_tx_buf[0] = (write_addr >> 16) & 0xff;

				g_tx_buf[7] = (write_addr >> 8) & 0xff;
				g_tx_buf[6] = (write_addr >> 0) & 0xff;
				g_tx_buf[5] = *(pInBuffer++);
				g_tx_buf[4] = *(pInBuffer++);
				...
				break;
			case 3:
				g_tx_buf[2] = BYTE_PROG;
				g_tx_buf[1] = (write_addr >> 16) & 0xff;
				g_tx_buf[0] = (write_addr >> 8) & 0xff;

				g_tx_buf[7] = (write_addr >> 0) & 0xff;
				g_tx_buf[6] = *(pInBuffer++);
				g_tx_buf[5] = *(pInBuffer++);
				g_tx_buf[4] = *(pInBuffer++);
			...
			while (data_len >= 4) {
				pTxBuffer[3] = *(pInBuffer++);
				pTxBuffer[2] = *(pInBuffer++);
				pTxBuffer[1] = *(pInBuffer++);
				pTxBuffer[0] = *(pInBuffer++);
		...
		debug("SF: 0x%p => cmd = { 0x%02x 0x%02x%02x%02x } chunk_len = %zu\n",
		      buf + actual, g_tx_buf[3], g_tx_buf[2], g_tx_buf[1], g_tx_buf[0],
			  chunk_len);

		ret = spi_xfer(flash->spi, (chunk_len + 4) << 3, g_tx_buf, g_rx_buf,
				SPI_XFER_BEGIN | SPI_XFER_END);
		if (ret != 0){
			printf("Error: %s(%d): failed\n",
					__FILE__, __LINE__);
		...
		if ((offset % imx_sf->params->block_size) == 0)
			printf(".");
...
static int spi_nor_flash_unlock_global_block_protection(struct spi_flash *flash)
{
	int ret;

	WRITE_ENABLE(flash);

	ret = UNLOCK_G_PROTECTION(flash);
	if (ret) {
		printf("Error: spi_nor_flash_unlock_global_block_protection(): %d\n", __LINE__);
	}
...
struct spi_flash *spi_flash_probe(unsigned int bus, unsigned int cs, unsigned int max_hz, unsigned int spi_mode)
{
	...
	if (0x26 == idcode[1]) {
		imx_sf->flash.write = spi_nor_flash_write_02;
	} else {
	...
	if (0x26 == idcode[1]) {
		spi_nor_flash_unlock_global_block_protection(&imx_sf->flash);
	} else {
		WRITE_ENABLE(&imx_sf->flash);
		ENABLE_WRITE_STATUS(&imx_sf->flash);
		spi_nor_write_status(&imx_sf->flash, 0);
	}
